...
def run_full_deepeval(df: pd.DataFrame, rubrics: List[Dict[str, Any]], mode: str = "single_turn", eval_model: Optional[str] = None,) -> pd.DataFrame:
    ...
    for i, row in df.iterrows():
        ...
        test_case = LLMTestCase(
            ...
            actual_output_text=row["actual_output"],
            expected_output_text=row["expected_output"]
        )
...
if mode == "🧩 Rubric Editor":
    ...
    with col1:
        uploaded_rubrics = st.file_uploader("Upload rubrics.json (optional)", type=["json"], accept_multiple_files=False)
